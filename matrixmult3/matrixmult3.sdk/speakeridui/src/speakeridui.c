...
int main()
{
    ...
    char message[] = "[ZYNC] Press 'm' to load pre-trained DNN SID model into BRAM, 's' to start speaker verification.  Any other key to repeat menu.\n";
	...
	if (Status != XST_SUCCESS) {
		...
		do {
			...
			case 's':
				...
				xil_printf("[ZYNC] Waiting for microphone samples over UART...\r\n");
				...
				xil_printf("[ZYNC] Computing MFCC features...\r\n");
				XGpio_DiscreteWrite(&Gpio, LED_CHANNEL, 4);
				for (Delay = 0; Delay < LED_DELAY*3; Delay++);
				xil_printf("[ZYNC] Done computing MFCC features...\r\n");

				xil_printf("[ZYNC] Evaluating frames on accelerator...\r\n");
